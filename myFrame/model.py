...
class DQN:
    def __init__(self, n_actions, n_features, learning_rate=0.01, reward_decay=0.9,
                 ...
                 batch_size=32, e_greedy_increment=0.001, output_graph=False, restore_DQN=False,
                 restore_file=None):
        ...
        if restore_DQN is False:
            self.sess.run(tf.global_variables_initializer())
        else:
            ckpt = tf.train.get_checkpoint_state(restore_file)
            if ckpt and ckpt.model_checkpoint_path:
                self.saver.restore(self.sess, ckpt.model_checkpoint_path)
            else:
                raise UserWarning('''Error, got a wrong restore_file input''')
        self.cost_his = []
    ...
    def save_net(self, name=None):
        ...
        if name:
            ...
            path = os.path.abspath('./model/{}'.format(time.strftime('%Y-%m-%d')))
            if not os.path.exists(path):
                os.mkdir(path)
            log_dir = path + '/{}/'.format(time.strftime('model-%Y%m%d-%H%M'))
            self.saver.save(self.sess, log_dir)
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            c_names, n_l1, w_initializer, b_initializer = \
                ...
                tf.random_normal_initializer(0., 0.001), tf.constant_initializer(0.1)  # config of layers
            ...
            with tf.variable_scope('l2'):
                ...
                b2 = tf.get_variable('b2', [1, self.n_actions], initializer=b_initializer,
                                     collections=c_names)
                self.q_eval = tf.sigmoid(tf.matmul(l1, w2) + b2)
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('l2'):
                ...
                b2 = tf.get_variable('b2', [1, self.n_actions], initializer=b_initializer,
                                     collections=c_names)
                self.q_next = tf.sigmoid(tf.matmul(l1, w2) + b2)
    ...
    def choose_action(self, observation):
        # to have batch dimension when feed into tf place holder
